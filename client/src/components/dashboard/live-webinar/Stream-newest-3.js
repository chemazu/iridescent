...
export default function Stream() {
  ...
  const [quizSubmission, setQuizSubmission] = useState(false);
  ...
  const handleCreatePoll = () => {
    setDefaultChat([
      ...defaultChat,
      { user: 1, type: "poll", title: pollTitle, options: pollOptions },
    ]);
    socket.emit(
      "message",
      { user: 1, options: pollOptions, type: "poll", timeStamp: Date.now() },
      roomid
    );
    setPollTitle("");
    setPollOptions(["", "", "", ""]);
  ...
  const handleQuizCreate = () => {
    ...
    setDefaultChat([
      ...defaultChat,
      { user: 1, quizHolder: newQuizHolder, type: "quiz" },
    ]);
  ...
  const handleScreenSharing = async () => {
    ...
    if (screenSharing) {
      console.log("first");
      socket.emit("screensharing", roomid);
      navigator.mediaDevices.getDisplayMedia(audioVisuals).then((stream) => {
        addVideoStream(myVideoRef.current, stream);
        connections.map((item) => {
          transmitScreenDisplay(item, stream);
        });
      });
