...
export const WithdrawHistoryTable = ({
    ...
    const handleGenerateWithdrawalReport = () => {
        ...
        const headers = [["Time", "Amount", "Recipient Bank", "Account Name", "Status"]];
    ...
    return <>
                 ...
                 </> : <>
                    {
                       withdrawHistory.map((withdrawItem) => 
                        <tr key={withdrawItem._id} className="tr-style">
                          <td>{moment(withdrawItem?.withdrawaldate).format('MMMM Do YYYY, h:mm:ss a')}</td>
        ...
        <div className="download-pdf-section text-center mt-3">
            ...
            block className="download-pdf__btn">Download PDF</Button>
        </div>
